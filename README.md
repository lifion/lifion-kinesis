...
| [options.largeItemThreshold]     | <code>number</code>               | <code>400</code>   | The size in KB above which an item should automatically be stored in s3.                                                                                                                                                                                                                  |
...
| [options.nonS3Keys]              | <code>Array.&lt;string&gt;</code> | <code>[]</code>    | If the `useS3ForLargeItems` option is set to `true`, the `nonS3Keys` option lists the keys that will be extracted from the large item stored in S3. The extracted keys will be persisted in DynamoDB so they are avaiable for queries.                                                    |
...
| [options.s3]                     | <code>object</code>               | <code>{}</code>    | The initialization options for the s3 client used to store large messages. In addition to `bucketName` and `endpoint`, it can also contain any of the [`AWS.S3` options](https://docs.aws.amazon.com/AWSJavaScriptSDK/latest/AWS/S3.html#constructor-property).                           |
| [options.s3.bucketName]          | <code>string</code>               |                    | The name of the table in which to store the state of consumers. If not provided, it defaults to "lifion-kinesis-state".                                                                                                                                                                   |
| [options.s3.endpoint]            | <code>string</code>               |                    | The S3 AWS endpoint to use.                                                                                                                                                                                                                                                               |
