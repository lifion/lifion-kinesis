...
import * as utilsStatic from './utils';
...
describe('lib/utils', () => {
  const { shouldBailRetry, transformErrorStack } = utilsStatic;

  test('the module exports the expected', () => {
    expect({ ...utilsStatic }).toEqual({
      getStackObj: expect.any(Function),
      shouldBailRetry: expect.any(Function),
      transformErrorStack: expect.any(Function)
    });
  });

  test('getStackObj returns undefined when CAPTURE_STACK_TRACE is unset', async () => {
    delete process.env.CAPTURE_STACK_TRACE;
    vi.resetModules();
    const utils = await import('./utils');
    expect(utils.getStackObj()).toBeUndefined();
  });

  test('getStackObj returns an object with a stack when CAPTURE_STACK_TRACE is on', async () => {
    process.env.CAPTURE_STACK_TRACE = true;
    vi.resetModules();
    const utils = await import('./utils');
    expect(utils.getStackObj().stack).toMatch(/^Error/);
  });
  ...
  `('shouldBailRetry should return $expected for errors $scenario', ({ code, expected }) => {
    const error = Object.assign(new Error('foo'), code && { code });
