...
async function deregisterStreamConsumer(props) {
  ...
  let { ConsumerStatus } = await describeStreamConsumer({ client, consumerName, streamArn });
  if (ConsumerStatus === 'DELETING') {
    ...
    do {
      ...
      ({ ConsumerStatus } = await describeStreamConsumer({ client, consumerName, streamArn }));
