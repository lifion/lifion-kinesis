...
module.exports.getShards = async ctx => {
  const { logger, client, streamName: StreamName } = ctx;
  logger.debug(`Retrieving the shards for the stream "${StreamName}"…`);
  const { Shards } = await client.listShards({ StreamName, MaxResults: 1000 }).promise();
  return Shards;
};
