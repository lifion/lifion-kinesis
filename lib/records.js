 ...
 */
function getRecordsDecoder({ compression, inputEncoding, logger, s3Client, useS3ForLargeItems }) {
  ...
  return async record => {
    ...
    try {
      ...
      if (useS3ForLargeItems && s3Client && s3Item) {
        ...
        const { Body, ContentType, ...s3Data } = await s3Client.getObject({
          Bucket: bucket,
          Key: key
        });
        return {
      ...
      logger.warn('Could not decode record:', err);
    }
...
class RecordsDecoder extends Transform {
   ...
   * @param {boolean} options.useS3ForLargeItems - Whether to automatically use an S3 bucket to store large items or not.
   */
  constructor({ compression, logger, s3Client, useS3ForLargeItems }) {
