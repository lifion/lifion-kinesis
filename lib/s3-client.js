/**
 * Module that wraps the calls to the AWS S3 client. Calls are wrapped so they can be
...
class S3Client {
  ...
  async getObject(...args) {
    const { client, retryOpts } = this.#data;
    const response = await retriableSdkCall(client, 'getObject', retryOpts, ...args);
    if (response && response.Body && typeof response.Body.transformToByteArray === 'function') {
      response.Body = Buffer.from(await response.Body.transformToByteArray());
    }
    return response;
  }
