...
describe('lib/lease-manager', () => {
  ...
  beforeEach(() => {
    LeaseManager.clearInstancesLeaseData('mock-consumer-group');
    LeaseManager.clearInstancesLeaseData('another-consumer-group');
  });
  ...
  test('lease manager handle useCachedAPIResponse', async () => {
    expect.assertions(4);
    const newOptions = Object.assign(options, { useCachedAPIResponses: true });
    const manager = new LeaseManager(newOptions);
    manager.cacheArnMetadata({ streamArn: {} });
    manager.cacheShardsMetadata({});
    await manager.start();
    expect(reconcile).not.toHaveBeenCalled();
    expect(checkIfStreamExists).not.toHaveBeenCalled();
    expect(getStreamShards).not.toHaveBeenCalled();
    expect(debug).toHaveBeenNthCalledWith(1, 'Attempting lease acquisition…');
  });
});
