...
import { checkIfStreamExists, getStreamShards } from './stream';

vi.mock('./stream', () => ({
  checkIfStreamExists: vi.fn(() => Promise.resolve({ streamArn: 'stream-arn' })),
  getStreamShards: vi.fn(() => Promise.resolve({}))
}));
...
describe('lib/lease-manager', () => {
  ...
  const releaseShardLease = vi.fn();
  const lockShardLease = vi.fn(() => Promise.resolve(true));
  const stateStore = {
