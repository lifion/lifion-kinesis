...
import { reportError, reportRecordSent, reportResponse } from './stats';
...
describe('lib/kinesis-client', () => {
  ...
  test('new instances of the module wrap instances of the AWS Kinesis client', () => {
  ...
  test('getCredentials resolves the credentials the client is configured with', async () => {
    recreateClients();
    await expect(client.getCredentials()).resolves.toEqual({
  ...
  test('retryOptions are merged into the retry behavior, overriding the defaults', async () => {
    const onRetry = vi.fn();
    client = new KinesisClient({ logger, retryOptions: { onRetry }, streamName: 'test-stream' });
    sdkClient = new Kinesis();
    error = Object.assign(new Error('foo'), { code: 'ProvisionedThroughputExceededException' });
    sdkClient.getRecords.mockImplementationOnce(() => Promise.reject(error));
  ...
  `('$methodName', ({ isRetriable, methodName }) => {
    ...
    if (methodName === 'describeStreamSummary') {
      test('describeStreamSummary should fallback to describeStream when operation is not supported', async () => {
        error = Object.assign(new Error('foo'), { code: 'UnknownOperationException' });
        sdkClient.describeStreamSummary.mockImplementationOnce(() => Promise.reject(error));

        const description = { foo: 'bar' };
        sdkClient.describeStream.mockImplementationOnce(() =>
          Promise.resolve({ StreamDescription: description })
        );
  ...
  describe('getRecords', () => {
    ...
    test('coerces each record Data into a Buffer', async () => {
      sdkClient.getRecords.mockImplementationOnce(() =>
        Promise.resolve({
          Records: [{ Data: new Uint8Array([104, 105]) }, { SequenceNumber: '1' }]
        })
      );
      const result = await client.getRecords({ ShardIterator: 'it' });
      expect(Buffer.isBuffer(result.Records[0].Data)).toBe(true);
      expect(result.Records[0].Data.toString()).toBe('hi');
      expect(result.Records[1].Data).toBeUndefined();
    });
  });
  ...
  describe('waitFor', () => {
    ...
    test('waitFor(streamExists) resolves with the waiter reason and reports a response', async () => {
      waitUntilStreamExists.mockResolvedValueOnce({
        reason: { StreamDescription: { foo: 'bar' } },
        state: 'SUCCESS'
      });
      await expect(client.waitFor('streamExists', { StreamName: 'x' })).resolves.toEqual({
        StreamDescription: { foo: 'bar' }
      });
      expect(waitUntilStreamExists).toHaveBeenCalledWith(
    ...
    test('waitFor(streamNotExists) uses the not-exists waiter', async () => {
      await client.waitFor('streamNotExists', { StreamName: 'x' });
      expect(waitUntilStreamNotExists).toHaveBeenCalled();
      expect(waitUntilStreamExists).not.toHaveBeenCalled();
    });

    test('waitFor reports and rethrows waiter errors', async () => {
      error = Object.assign(new Error('boom'), { code: 'X' });
      waitUntilStreamExists.mockRejectedValueOnce(error);
      await expect(client.waitFor('streamExists', {})).rejects.toThrow('boom');
      expect(reportError).toHaveBeenCalledWith('kinesis', error, 'test-stream');
    });
  });
