...
describe('lib/kinesis-client', () => {
  ...
  `('$methodName', ({ isRetriable, methodName }) => {
    ...
    if (isRetriable) {
      test(`${methodName} retries errors from the wrapped SDK and logs as debug`, async () => {
      ...
      test(`${methodName} retries errors from the wrapped SDK and logs as a warning`, async () => {
        error = Object.assign(new Error('foo'), { code: 'NetworkingError' });
        sdkClient[methodName].mockImplementationOnce(() => ({
          promise: () => Promise.reject(error)
        }));
        const promise = client[methodName]({});
