...
vi.mock('util', async () => ({ ...(await vi.importActual('util')), promisify: () => wait }));
...
describe('lib/stream', () => {
  ...
  const addTagsToStream = vi.fn();
  const createStream = vi.fn();
  const describeStreamSummary = vi.fn();
  const listShards = vi.fn();
  const listStreamConsumers = vi.fn();
  const listTagsForStream = vi.fn();
  const registerStreamConsumer = vi.fn();
  const startStreamEncryption = vi.fn();
  const waitFor = vi.fn();
  const client = {
    addTagsToStream,
    createStream,
