...
class PostProcess extends Writable {
  constructor({ pushToStream, shardId, stateStore }) {
    super({ objectMode: true });
    Object.assign(internal(this), { pushToStream, shardId, stateStore });
  }

  async _write(chunk, encoding, callback) {
    const { pushToStream, shardId, stateStore } = internal(this);
    ...
    do {
      ...
      try {
        await asyncPipeline([
          ...
          new PostProcess({ pushToStream, shardId, stateStore })
        ]);
      } catch (err) {
        if (err.isRetryable) {
