...
class PostProcess extends Writable {
  constructor({ client, logger, pushToStream, shardId, stateStore, streamName }) {
    super({ objectMode: true });
  ...
  async _write(chunk, encoding, callback) {
    const { client, logger, pushToStream, shardId, stateStore, streamName } = internal(this);
    ...
    do {
      ...
      try {
        await asyncPipeline([
          ...
          new PostProcess({ client, logger, pushToStream, shardId, stateStore, streamName })
        ]);
      } catch (err) {
        if (err.isRetryable) {
