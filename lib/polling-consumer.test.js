...
describe('lib/polling-consumer', () => {
  ...
  test('the consumer keeps polling with a re-created iterator if the previous one expires', done => {
    ...
    setTimeout.mockImplementationOnce(() => {
      try {
        ...
        expect(getRecords).toHaveBeenNthCalledWith(1, { Limit: 1000, ShardIterator: 'iterator-1' });
        expect(getRecords).toHaveBeenNthCalledWith(2, { Limit: 1000, ShardIterator: 'iterator-2' });
        expect(getRecords).toHaveBeenNthCalledWith(3, { Limit: 1000, ShardIterator: 'iterator-3' });
