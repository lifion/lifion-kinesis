...
describe('lib/state-store', () => {
  ...
  test("starting the state store shouldn't be a problem if the DynamoDB table exists", async () => {
    const store = new StateStore(Object.assign({}, options, { dynamoDb: { tags: undefined } }));
    const client = new DynamoDbClient();
  ...
  test('clearOldConsumers removes late-heartbeat consumers and references to them', async () => {
    ...
    expect(update).toHaveBeenNthCalledWith(1, {
      ...
      ExpressionAttributeNames: { '#0': 'consumer-2', '#a': 'consumers', '#b': 'version' },
    ...
    expect(update).toHaveBeenNthCalledWith(2, {
      ExpressionAttributeNames: {
        '#0': 'enhanced-consumer-1',
        '#1': 'enhanced-consumer-2',
        '#a': 'enhancedConsumers',
        '#b': 'isUsedBy',
        '#c': 'version'
      },
      ExpressionAttributeValues: { ':0': '0002', ':1': '0003', ':z': null },
      Key: { consumerGroup: 'test-group', streamName: 'test-stream' },
      UpdateExpression: 'SET #a.#0.#b = :z, #a.#0.#c = :0, #a.#1.#b = :z, #a.#1.#c = :1'
    });
    expect(update).toHaveBeenCalledTimes(2);
    expect(debug).toHaveBeenNthCalledWith(2, 'Cleared 1 old consumer(s).');
    expect(debug).toHaveBeenNthCalledWith(3, 'Released usage of 2 enhanced consumer(s).');
  ...
  test('clearOldConsumers clears enhanced consumers when no consumers exist', async () => {
    ...
    get.mockResolvedValueOnce({
      Item: {
        consumers: {},
        enhancedConsumers: { 'enhanced-consumer-1': { isUsedBy: 'consumer-1' } },
        version: '0000'
      }
    });
    await expect(store.clearOldConsumers(1000)).resolves.toBeUndefined();
    expect(update).toHaveBeenNthCalledWith(1, {
      ...
      ExpressionAttributeValues: { ':0': '0001', ':z': null },
      Key: { consumerGroup: 'test-group', streamName: 'test-stream' },
      UpdateExpression: 'SET #a.#0.#b = :z, #a.#0.#c = :0'
    });
    expect(update).toHaveBeenCalledTimes(1);
    expect(debug).toHaveBeenNthCalledWith(2, 'Released usage of 1 enhanced consumer(s).');
  ...
  test('clearOldConsumers throws if clearing enhanced consumers throws', async () => {
    ...
    get.mockResolvedValueOnce({
      Item: {
        consumers: {},
        enhancedConsumers: { 'enhanced-consumer-1': { isUsedBy: 'consumer-2' } },
        version: '0000'
      }
    });
    update.mockRejectedValueOnce(new Error('foo'));
  ...
  test('getAssignedEnhancedConsumer marks the consumer as inactive on no assignment', async () => {
    ...
    expect(warn).toHaveBeenNthCalledWith(1, "Couldn't lock an enhanced fan-out consumer.");
    expect(warn).toHaveBeenCalledTimes(1);
    expect(update).toHaveBeenCalledWith({
      ExpressionAttributeNames: { '#a': 'consumers', '#b': 'test-id', '#c': 'isActive' },
      ExpressionAttributeValues: { ':z': false },
      Key: { consumerGroup: 'test-group', streamName: 'test-stream' },
  ...
  test('getAssignedEnhancedConsumer locks an enhanced consumer if not assigned yet', async () => {
    ...
    expect(update).toHaveBeenNthCalledWith(1, {
      ...
      ExpressionAttributeValues: { ':x': 'test-id', ':y': '0001', ':z': '0000' },
      Key: { consumerGroup: 'test-group', streamName: 'test-stream' },
  ...
  test('getAssignedEnhancedConsumer works when auto shard assignment is off', async () => {
    ...
    expect(update).toHaveBeenNthCalledWith(1, {
      ...
      ExpressionAttributeValues: { ':w': {}, ':x': 'test-id', ':y': '0001', ':z': '0000' },
      Key: { consumerGroup: 'test-group', streamName: 'test-stream' },
