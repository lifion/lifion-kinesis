...
import { confirmTableTags, ensureTableExists } from './table';
...
vi.mock('./dynamodb-client', () => {
  ...
  return {
    default: vi.fn(function () {
      return { delete: deleteMock, get, put, stop, update };
    })
  };
});
...
describe('lib/state-store', () => {
    ...
    get.mockResolvedValueOnce({
      Item: {
        consumers: {
          ...
          'consumer-3': { heartbeat: new Date('2100-01-01') }
        },
        enhancedConsumers: {
  ...
  test('storeShardCheckpoint should be called if timestamp is not a date instance, timestamp stored as null', async () => {
    const store = new StateStore(options);
    ...
    expect(update).toHaveBeenNthCalledWith(1, {
      ...
      UpdateExpression: 'SET #a.#b.#c = :x, #a.#b.#d = :y, #a.#b.#e = :z'
    });
    expect(update).toHaveBeenCalledTimes(1);
  });
  ...
  test('storeShardCheckpoint should be called if timestamp is null, timestamp stored as null', async () => {
    ...
    await store.start();
    await expect(
      store.storeShardCheckpoint('shard-0001', '1', '#a', { '#a': 'a' }, null)
    ).resolves.toBeUndefined();
    const { update } = new DynamoDbClient();
    expect(update).toHaveBeenNthCalledWith(1, {
      ExpressionAttributeNames: {
  ...
  test('storeShardCheckpoint updates the checkpoint of a shard', async () => {
    ...
    expect(update).toHaveBeenNthCalledWith(1, {
      ...
      ExpressionAttributeValues: {
        ':x': approxArrivalTimestamp.toISOString(),
        ':y': '1',
        ':z': '0001'
      },
      Key: {
