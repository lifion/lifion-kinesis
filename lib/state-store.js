...
const DynamoDBProxy = require('./dynamodb-proxy');
...
class StateStore {
   ...
   */
  constructor({ consumerGroup, consumerId, dynamoDbOptions, logger, streamName }) {
  ...
  async start() {
    ...
    const { awsOptions, consumerGroup, logger, streamName, tableName, tags } = privateProps;

    const client = new DynamoDBProxy({ awsOptions, tableName });
    privateProps.client = client;

    let tableArn = await checkIfTableExists(client, logger, tableName);
    if (!tableArn) tableArn = await createTable(client, logger, tableName);
