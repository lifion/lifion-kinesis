...
class StateStore {
   ...
   */
  constructor({ consumerGroup, consumerId, dynamoDb, logger, streamCreatedOn, streamName }) {
  ...
  async markShardAsDepleted(shardsData, parentShardId) {
    ...
    await client.update({
      ...
      UpdateExpression: `SET ${[
        ...
        ...childrenShards.map(
          (childShard, index) =>
            `#a.#${index}.#e = :${index * 2}, #a.#${index}.#d = :${index * 2 + 1}`
        )
      ].join(', ')}`,
  ...
  async getOwnedShards() {
    ...
    return shardIds
      ...
      .reduce((obj, shardId) => {
        ...
        return { ...obj, [shardId]: { checkpoint, version } };
      }, {});
  }
}
