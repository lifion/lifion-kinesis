...
import { BAIL_RETRY_LIST, FORCED_RETRY_LIST } from './constants.js';
...
function transformErrorStack(err, stackObj) {
  ...
  awsError.requestId ??= awsError.$metadata?.requestId;
  awsError.statusCode ??= awsError.$metadata?.httpStatusCode;
  if (!stackObj) return awsError;
  const { code, message, requestId, statusCode } = awsError;
...
export { getStackObj, shouldBailRetry, transformErrorStack };
