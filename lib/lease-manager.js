...
async function checkStream(props, leaseManagerInstance) {
  const arnMetadata = await checkIfStreamExists(props);
  leaseManagerInstance.cacheArnMetadata(arnMetadata);
  return arnMetadata;
}

async function checkStreamShards(props, leaseManagerInstance) {
  const shardsMetadata = await getStreamShards(props);
  leaseManagerInstance.cacheShardsMetadata(shardsMetadata);
 ...
 * - Acquiring or renewing the lease would go beyond the maximum count of allowed active leases.
 *
...
class LeaseManager {
  /**
   * Given a specific consumer group, this static function clears all AWS API metadata responses from all instances of this class
   *
   * @param {string} consumerGroup - The consumer group to refresh lease manager instances
   */
  static clearInstancesLeaseData(consumerGroup) {
    for (const [key, value] of LeaseManager.instanceMap) {
      if (key.startsWith(consumerGroup)) {
        value.streamArnMetadata = undefined;
        value.streamShardsMetadata = undefined;
      }
    }
  }
   ...
   * @param {string} [options.consumerGroup] - The general consumer group name for this lease manager
   ...
   * @param {string} options.streamName - The name of the Kinesis stream.4j7
   * @param {string} options.useAutoShardAssignment - Set to `true` to automatically assign
   ...
   * @param {boolean} [options.useCachedAPIResponses] - Flag to indicate whether to cache all AWS API calls for stream summaries and shard configuration. Setting this to true, reduces the chattiness between consumers and AWS APIs that can leas to potential LimitExceededException errors when many consumers exist in the same AWS account
   */
  constructor(options) {
    const {
      client,
    ...
    this.streamArnMetadata = undefined;
    this.streamShardsMetadata = undefined;
    LeaseManager.instanceMap.set(`${consumerGroup}-${streamName}`, this);
  }
  ...
  async start() {
    ...
    const acquireLeases = async () => {
      ...
      try {
        ...
        const { streamArn } =
          useCachedAPIResponses && this.streamArnMetadata
            ? this.streamArnMetadata
            : await checkStream(privateProps, this);
        if (!streamArn) {
        ...
        const shards =
          useCachedAPIResponses && this.streamShardsMetadata
            ? this.streamShardsMetadata
            : await checkStreamShards(privateProps, this);
  ...
  /**
   * Sets this instance Kinesis Stream ARN metadata to be reused
   *
   * @param {Object} metadata - The ARN metadata object to assign
   */
  cacheArnMetadata(metadata) {
    this.streamArnMetadata = metadata;
  }

  /**
   * Sets this instance Kinesis Stream shard configuration metadata to be reused
   *
   * @param {Object} metadata - The shard configuration metadata object to assign
   */
  cacheShardsMetadata(metadata) {
    this.streamShardsMetadata = metadata;
  }
}

LeaseManager.instanceMap = new Map();
