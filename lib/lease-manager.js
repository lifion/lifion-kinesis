...
class LeaseManager {
  ...
  async start() {
    ...
    const acquireLeases = async () => {
      let nextDelay = privateProps.leaseAcquisitionInterval || ACQUIRE_LEASES_INTERVAL;
      ...
      try {
        ...
        if (useEnhancedFanOut) {
          ...
          if (!consumerArn) {
            privateProps.timeoutId = setTimeout(
              acquireLeases,
              privateProps.leaseAcquisitionRecoveryInterval || ACQUIRE_LEASES_RECOVERY_INTERVAL
            );
            return;
          }
        }
