...
import { S3, mockClear } from '@aws-sdk/client-s3';
...
describe('lib/s3-client', () => {
  ...
  test('new instances of the module wrap instances of the AWS S3 client', () => {
    ...
    ({ defaultValue, expectedRetries, isRetriable, methodName, notFoundErrorCode }) => {
      ...
      if (isRetriable) {
        test(`retries errors from the wrapped SDK`, async () => {
          ...
          sdkClient[methodName].mockImplementationOnce(() => Promise.resolve(defaultValue));
  ...
  describe('getObject body coercion', () => {
    const client = new S3Client({ bucketName: 'test bucket name', logger });
    const sdkClient = new S3();

    beforeEach(() => {
      mockClear();
      sdkClient.getObject.mockClear();
    });

    test('coerces a streaming Body into a Buffer', async () => {
      sdkClient.getObject.mockImplementationOnce(() =>
        Promise.resolve({
    ...
    test('leaves a non-streaming Body untouched', async () => {
      sdkClient.getObject.mockImplementationOnce(() =>
        Promise.resolve({ Body: 'already-a-string' })
      );
      const result = await client.getObject({ Bucket: 'b', Key: 'k' });
